...
export class SimulationAgent extends TypedEventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    this.setState({ 
      ...
      lastSimulatedUserResponse: null,
      currentTurn: 0,
      isComplete: false
    });
    await this.nextTurn();
  ...
  async nextTurn(): Promise<SimulationAgentState> {
    ...
    this.setState({ 
      lastSimulatedUserResponse: response,
      lastAgentResponse: agentResponse,
      currentTurn: this.state.currentTurn + 1,
      // If the "simulated user" returns a stop token, the simulation is complete
      isComplete: (this.state.currentTurn + 1) >= this.maxTurns || ('stopTokenReturned' in response && response.stopTokenReturned)
    });
